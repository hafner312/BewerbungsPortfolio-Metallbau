 ...
 * die Karten auf einen Blick unterscheiden lassen. Die Technologie-Chips
...
/** Eine einzelne Fähigkeiten-Karte für eine Kategorie */
function SkillCard({ cat, delay }: { cat: Skill['category']; delay: number }) {
...
/** Eigener Abschnitt für die Metallbau-Kompetenzen, bewusst vor den IT-Fähigkeiten */
export function MetallbauSkills() {
  return (
    <section id="metallbau-skills" className="py-24 px-6 bg-[var(--color-bg-surface)]">
      <div className="max-w-6xl mx-auto">
        <SectionHeading
          title="Metallbau"
          subtitle="Kompetenzen aus über zehn Jahren Berufserfahrung"
        />

        <div className="max-w-xl mx-auto">
          <SkillCard cat="metallbau" delay={0} />
        </div>
      </div>
    </section>
  )
...
/** Abschnitt für die IT-Fähigkeiten, getrennt von den Metallbau-Kompetenzen */
export function Skills() {
  const itCategories: Skill['category'][] = ['frontend', 'backend', 'devops', 'tools']

  return (
    <section id="skills" className="py-24 px-6 bg-[var(--color-bg-base)]">
      <div className="max-w-6xl mx-auto">
        <SectionHeading
          title="IT-Fähigkeiten"
          subtitle="Technologien, mit denen ich aus meiner Zusatzausbildung arbeite"
        />

        <div className="grid md:grid-cols-2 gap-6">
          {itCategories.map((cat, i) => (
            <SkillCard key={cat} cat={cat} delay={i * 0.08} />
          ))}
        </div>
      </div>
    </section>
